...
export class InitialMigration1768188602181 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`students\` (\`email\` varchar(255) NOT NULL, \`is_suspended\` tinyint NOT NULL DEFAULT 0, PRIMARY KEY (\`email\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`teachers\` (\`email\` varchar(255) NOT NULL, PRIMARY KEY (\`email\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`teacher_students\` (\`student_email\` varchar(255) NOT NULL, \`teacher_email\` varchar(255) NOT NULL, INDEX \`IDX_b66b0531c641ae33dca35eb314\` (\`student_email\`), INDEX \`IDX_81753dd9ba40e57515fdf8054f\` (\`teacher_email\`), PRIMARY KEY (\`student_email\`, \`teacher_email\`)) ENGINE=InnoDB`);
